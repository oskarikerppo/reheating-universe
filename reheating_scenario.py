...
def rho_phi(t, t_0, n, xi, m, l, b):
	return (scale_factor(t)**(-3.0))*np.exp(-f(t, t_0, n, xi, m, l))*integrate.quad(lambda x: (scale_factor(x)**3.0)*Gamma_phi(x, m, b)*np.exp(f(x, t_0, n, xi, m, l)), t_0, t, epsabs=eabs, epsrel=erel, limit=lmt)[0]
...
def rho_phi_mat(t, n, xi, m, l, eq_time, rho_eq):
	return ((eq_time/t)**2.0)*(np.exp(-f(t, eq_time, n, xi, m, l)) * rho_eq)
...
def rho_phi_rad(t, n, xi, m, l, eq_tau, rho_tau):
	return ((eq_tau/t)**(3/2))*(np.exp(-f(t, eq_tau, n, xi, m, l)) * rho_tau)
